...
# 자식 클래스 _ DroneRobot, CleanRobot
...
class DroneRobot(Robot):
    ...
    def __init__(self, name, power, battery, height):
        # super() : 자식 클래스의 생성자에서 부모 클래스를 가져오는 메소드
...
'''
    ...
    - 정의할 내용
        ...
        - move() 메소드 오버라이딩
            ...
            * 기능 : 
                - 이동을 할 때마다 먼저 bin 을 1 씩 증가시킨다 (먼지 흡입)
                - 먼지통이 꽉차면, "더 이상 청소할 수 없습니다. 먼지통을 비워주세요..." 출력
        
        - mapping() 메소드  * '청소할 영역을 기억합니다'
        
        - vacate() 메소드   * '먼지통 비우는 기능'
            * 출력 : '먼저통을 비웁니다'
            * 먼지통 비우는 기능
...
class CleanRobot(Robot):
    ...
    def move(self, direction):
        ...
        if( self.bin >= CleanRobot.max_bin ):
            print('더 이상 청소할 수 없습니다. 먼지통을 비워주세요...')
            return
        self.bin += 1       # 먼지 흡입
